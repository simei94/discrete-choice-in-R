...
# Set the parameters: 
# Experiment 1: mu = beta = 0 -> null model
mu <- 0
beta <- 0
...
# => max likelihood L = 0.0763 with beta = -0.65 and mu = 0.1
...
# now, walk is the reference mode
# Note: use chunk option results="asis" to display latex output in pdf 
model2 <- mlogit(f2,
                 mc_commute_long,
...
median(mc_commute_predict$time,
       na.rm = TRUE)
# median trip duration is 10 min
